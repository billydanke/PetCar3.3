...
class TtsController:
    ...
    async def handle_command(self, websocket: ServerConnection, text: str) -> None:
        ...
        self.state.last_text = normalized_text
        self._ensure_worker()
        await self._queue.put(normalized_text)
        self.logger.info("TTS queued: %s", normalized_text)
        await websocket.send("a t ok")
